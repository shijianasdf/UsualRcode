##############################
#software packages
##############################

#everything 查找本地文件神器
#冰点文库 免费下载百度文库 道客巴巴文库神器
#Xmind思维导图 coggle
#git svn 版本控制系统
#Chrono 谷歌下载管理器
#掘金，scholarscope，医学文献助手，pubmedplus, wappalyzer, whatruns, (30 seconds of knowledges), vip视频解析, Wap Scraper, octotree,谷歌插件
#30 Seconds of Knowledge;Diigo Web Collector - Capture and Annotate3.4.3,enhanced github,onetab,插件大师,谷歌访问助手,油猴;
#护眼宝
#Navicat Mysqlworkbench  Mysql GUI
#eclipse Apache Tomcat 
#Webstrom Rstudio
#Robo3T Studio3T mongoDB GUI
#有道云笔记 


##############################
#有用的资源站
##############################
#兰客导航
#优设导航
#虫部落
#传送门 微信公众号推送
#Hippter PPT资源
#1)开源中国 2)ITeye 3)InfoQ 4)Stack Overflow 5)HackerRank 6)Programming by Doing 7)Hacker News 8)coursera 9)mooc 10)极客学院 11)办公资源网
#11)网易公开课 12)腾讯课堂 13)爱课程 14)课堂在线
#CODELF  代码命名
#wappalyzer 网站和插件 解析网站技术

##------------------------------------------
#bioinformatics resource
##------------------------------------------
#http://genomicsclass.github.io/book/
#https://bioconductor.github.io/BiocWorkshops/
#http://kasperdanielhansen.github.io/genbioconductor/
#https://www.youtube.com/results?search_query=bioconductor
#http://www.youtube.com/user/RafalabChannel?feature=watch
#http://manuals.bioinformatics.ucr.edu/home/R_BioCondManual
#http://www.vincebuffalo.com/2012/03/12/23andme-gwascat.html
#http://search.bioconductor.jp/
#http://watson.nci.nih.gov/~sdavis/
#https://bioinformatics.ca/
#biostar SEQanwsers
#https://github.com/hbctraining
#简书，知乎
#生信自学网
#腾讯课堂，网易云课
#生信技能树
#生信菜鸟团


...
######----------- 首先创建对象 newCellDataSet -----------#######
{
  ...
  sc_cds <- newCellDataSet(
    as.matrix(counts,"sparseMatrix"), 
    phenoData = pd,
    featureData =fd,
    expressionFamily = negbinomial.size(),
    lowerDetectionLimit=1)
  sc_cds@featureData@data
  str(sc_cds)
...
######---------- Clustering cells without marker genes --------------#########
{
  ...
  #We can filter genes based on average expression level, 
...
#####------------  最后推断发育轨迹 -------------#############
{
  ...
  # 可以很明显看到细胞的发育轨迹
  ## 这里可以展现marker基因在发育轨迹推断的效果，本例子随便选取了6个差异表达基因。
